$.afui.useOSThemes=false;
    ...
    $(document).ready(function(){
		...
		$("#br_qty_show_btn").show();	
		$('#tbl_dcr_br_qty_show').append(localStorage.brand_show_li);
...
//	$("#wait_image_tbl_docBrAdd").show();
	...
	$.ajax(localStorage.base_url+'get_doc_br_data?cid='+localStorage.cid+'&rep_id='+localStorage.user_id+'&rep_pass='+localStorage.user_pass+'&synccode='+localStorage.synccode+'&doc_Id='+doc_Id+'&doc_name='+doc_name,{
		...
		timeout: 30000,
		error: function(xhr) {
		$("#btn_show_doctor_brand").show();
		$("#wait_image_visit_submit_doc").hide();
	//	$("#wait_image_tbl_docBrAdd").hide();
...
//		show_br_dcr_str=show_br_dcr_str+'<option value="">Select Brand</option>'
	//	alert(show_br_dcr_str)
		var brand_bDoc_list=''
		 var br_qty	= ''
	...
	//	alert(localStorage.show_br_dcr_str)
...
function page_doctor_profile() {
	...
	$("#doctor_prof").val(localStorage.report_url+'doc_info?cid='+localStorage.cid+'&rep_id='+localStorage.user_id+'&rep_pass='+localStorage.user_pass+'&synccode='+localStorage.synccode+'&route='+market_Id+'&docId='+visitDocId)
		$.ajax(localStorage.report_url+'doc_info?cid='+localStorage.cid+'&rep_id='+localStorage.user_id+'&rep_pass='+localStorage.user_pass+'&synccode='+localStorage.synccode+'&route='+market_Id+'&docId='+visitDocId,{
								...
								$('#collar_size_combo').append('<option value="'+collar_size+'" >'+collar_size_combo+'</option>');
...
function docProfileSubmit() {
	...
	//================================
	dMarDay=$("#dMarDay").val()
	collar_size_combo=$("#collar_size_combo").val()
	dc1Day=$("#dc1Day").val()
	dc2Day=$("#dc2Day").val()
	//==========================
	dTidP=$("#dTidP").val()
...
//	alert(localStorage.visit_client_show)
//			$.ajax({
//				 type: 'POST',
...
function doc_cat_brand() {
	...
	$.ajax(localStorage.base_url+'getDocItemBrandList?cid='+localStorage.cid+'&rep_id='+localStorage.user_id+'&rep_pass='+localStorage.user_pass+'&synccode='+localStorage.synccode,{
								// cid:localStorage.cid,rep_id:localStorage.user_id,rep_pass:localStorage.user_pass,synccode:localStorage.synccode,
								type: 'POST',
								timeout: 30000,
								error: function(xhr) {
								...
								success:function(data, status,xhr){	
									...
									var br_doc = resultArray[1];
									localStorage.br_doc=br_doc;
									$("#wait_image_brand_submit_doc").hide();		
...
//						//					-----------------------------------
//														
								var brandDocList = br_doc.split('<rd>');
								//alert()
								var brandDocListShowLength=brandDocList.length	
								var br_dcr_combo_str='<select name="brand_dcr_com" id="brand_dcr_com" >'
								br_dcr_combo_str=br_dcr_combo_str+'<option value="">Select Brand</option>'
								var brand_bDoc_list=''
								 var br_qty	= ''
								for (var i=0; i < brandDocListShowLength; i++){	
								...
								var i_text=i.toString();
								var bDocValueArray = brandDocList[i].split('<fd>');
								var bDocID=bDocValueArray[0];
								var bDocName=bDocValueArray[1];
								
								var cmb_br_dcr_id = 'c_'+bDocID+'_'+i_text;	
											
									br_dcr_combo_str=br_dcr_combo_str +'<option name="'+cmb_br_dcr_id+'" id="'+cmb_br_dcr_id+'" value="'+bDocID+'|'+bDocName+'" >'+bDocName+'</option>'			  	


								}
								br_dcr_combo_str=br_dcr_combo_str+'</select>'
								
								
								localStorage.br_dcr_combo_str=br_dcr_combo_str
								
							//	alert(localStorage.br_dcr_combo_str)
								
								$('#doctor_brand_combo_div').empty();
								$('#doctor_brand_combo_div').append(localStorage.br_dcr_combo_str).trigger('create');
...
function show_tbl_br(){	
	
	var brand_dcr_com_val = $("#brand_dcr_com").val();
	var br_qty_val = $("#br_qty_id").val();
	
	//alert(br_qty_val)

	if (br_qty_val>0) {
			var br_sto_Row = localStorage.docBrandList ;
			var strShow=brand_dcr_com_val
			var rep_val =strShow+'<fdfd>'+br_qty_val+'<rdrd>' ;
		...
		//	alert (br_sto_Row)
			...
			if ((br_sto_Row.indexOf(strShow)!=-1)  ){
			    submitbrList=br_sto_Row.split('<rdrd>')
				
				for (var k=0; k < submitbrList.length-1; k++){	
					var strShowLoop = 	submitbrList[k]	
					
						
					if ((strShowLoop.indexOf(strShow)!=-1)  ){	
	
							br_sto_RowGet=br_sto_Row.replace(submitbrList[k]+'<rdrd>',rep_val)
							localStorage.docBrandList=br_sto_RowGet
						}
			...
			localStorage.docBrandList=localStorage.docBrandList+brand_dcr_com_val+'<fdfd>'+br_qty_val+'<rdrd>'
			}
...
//======Table
	...
	//alert(docBrandListSingleRow)
		var i_text=i.toString();
		var i_brId = 'brId_'+i_text;
		var bDocValueArray = docBrandListSingleRow[i].split('<fdfd>');


		var docBrandId = docBrandListSingleRow[i].split('<fdfd>')[0].split('|')[0] ;
		var docBrandName = docBrandListSingleRow[i].split('<fdfd>')[0].split('|')[1] ;
	//	alert(docBrandName)
	...
	$("#tbl_dcr_br_qty_show").html(showBr);

	  }	//br_qty_val
	}

function br_delete(i){
	var single_br_doc=localStorage.docBrandList
	var br_data_show=single_br_doc.split('<rdrd>')[i];

	var final_br_data=single_br_doc.replace(br_data_show+'<rdrd>','');

	localStorage.docBrandList=final_br_data;



	var docBrListSingleRow = localStorage.docBrandList.split('<rdrd>') ;
//	alert(docBrandListSingleRow);
	var showBr_tbl = '<table width="100%" border="1" id="order_tbl" cellpadding="1" cellspacing="1" style="border-radius:5px;"><tr style="border-bottom:1px solid #D2EEE9;"><td width="60px" style="text-align:center; padding-left:5px;">Brand</td><td width="60px" style="text-align:center; padding-left:5px;">Quantity</td><td width="60px" style="text-align:center; padding-left:5px;">Delete</td></tr>' ;
//	if (resultArray[0]=='FAILED'){
//	
//	}
	for (var j=0; j < docBrListSingleRow.length-1; j++){	
	//alert(docBrandListSingleRow)
		var i_text=j.toString();
		var i_brId = 'brId_'+i_text;
		//alert (docBrListSingleRow[j])
		//var bDocValueArray = docBrListSingleRow[j].split('<fdfd>');
	

		var docBrandId = docBrListSingleRow[j].split('<fdfd>')[0].split('|')[0] ;
		var docBrandName = docBrListSingleRow[j].split('<fdfd>')[0].split('|')[1] ;
		var brQty = docBrListSingleRow[j].split('<fdfd>')[1] ;

	//	alert(bDocValueArray);
		showBr_tbl = showBr_tbl+'<tr style="border-bottom:1px solid #D2EEE9;">'+'<td width="60px" style="text-align:center; padding-left:5px;">'+docBrandName+'<input type = "hidden" id = "'+i_brId+'" name =  "'+i_brId+'" value =  "'+docBrandId+'"></td><td width="60px" style="text-align:center; padding-left:5px;">'+brQty+'</td><td width="60px" style="text-align:center; padding-left:5px;"><input type="submit" style="height:30px" name="br_qty_del_btn" id="br_qty_del_btn" onClick="br_delete('+j+')" value="X"></td></tr>'
		
		
		
	}
	
	showBr_tbl=showBr_tbl+'</table>'
	
	$("#tbl_dcr_br_qty_show").html(showBr_tbl);

}

function docBrSubmit() {
	//			//// ajax-------
	...
	$.ajax(localStorage.base_url+'syncBrSubmitData?cid='+localStorage.cid+'&rep_id='+localStorage.user_id+'&rep_pass='+localStorage.user_pass+'&synccode='+localStorage.synccode+'&docBrandList='+localStorage.docBrandList+'&doc_Id='+doc_Id+'&doc_name='+doc_name+'&market_Id='+market_Id+'&market_name='+market_name,{
								...
								if (resultArray[0]=='FAILED'){
											$("#error_brand_doc").text(resultArray[1]);	
